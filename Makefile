...
build:
	...
	@go build -ldflags="-s -w" -o srt ./cmd/srt
	@echo "✓ Built srt"

# Build universal binary (ARM64 + AMD64)
universal:
	@echo "Building for ARM64..."
	@GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w" -o srt-arm64 ./cmd/srt
	@echo "Building for AMD64..."
	@GOOS=darwin GOARCH=amd64 go build -ldflags="-s -w" -o srt-amd64 ./cmd/srt
	@echo "Creating universal binary..."
	@lipo -create srt-arm64 srt-amd64 -output srt
	@rm srt-arm64 srt-amd64
	@echo "✓ Built srt (universal binary)"

# Install to /usr/local/bin
install: universal
	@echo "Installing to /usr/local/bin..."
	@install -m 755 srt /usr/local/bin/srt
